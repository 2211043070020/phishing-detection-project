(function() {
    ...
    function analyzeDNSForPhishing(dnsData) {
        let riskScore = 0;
        
        if (!dnsData || !dnsData.records) {
            return 1; // High risk if no DNS data
        }
        
        try {
            // Check TXT records for verification
            const txtRecords = dnsData.records.TXT || [];
            const verificationKeywords = ['verification', 'spf1', 'dmarc', 'domainkey', 'dkim'];
            const verificationCount = txtRecords.filter(txt => 
                verificationKeywords.some(keyword => 
                    String(txt).toLowerCase().includes(keyword)
                )
            ).length;
            
            riskScore += verificationCount >= 2 ? -2 : 2;
            
            // Check for enterprise services
            const enterpriseServices = ['cisco', 'microsoft', 'google', 'atlassian', 'cloudflare', 'amazon'];
            let hasEnterpriseService = txtRecords.some(txt =>
                enterpriseServices.some(service =>
                    String(txt).toLowerCase().includes(service)
                )
            );
            
            riskScore += hasEnterpriseService ? -2 : 1;
            
            // Check MX records
            const mxRecords = dnsData.records.MX || [];
            if (mxRecords.length > 0) {
                const mxProvider = String(mxRecords[0]?.exchange || '').toLowerCase();
                const legitimateProviders = ['google', 'microsoft', 'protonmail', 'outlook', 'gmail'];
                riskScore += legitimateProviders.some(provider => 
                    mxProvider.includes(provider)) ? -2 : 1;
            } else {
                riskScore += 2; // No email infrastructure is suspicious
            }
            
            // Check A records (should exist for legitimate sites)
            ...
            console.error('Error analyzing DNS data:', error);
            return 1;
        }
        
        return riskScore < 0 ? -1 : 1;
    }
    ...
    function cleanDomainName(domain) {
        ...
        if (domain.endsWith('.ai')) {
            domain = domain.replace('.ai', '.com');
        }
        
        return domain;
    }
    
    async function checkDNSRisk(domain) {
        try {
            const cleanDomain = cleanDomainName(domain);
            if (!cleanDomain) return 1;
            
            const dnsData = await fetchDNSData(cleanDomain);
            return analyzeDNSForPhishing(dnsData);
        } catch (error) {
            console.error('DNS risk check failed:', error);
            return 1; // Default to risky if check fails
        }
    }
    ...
    if (chrome && chrome.runtime) {
        chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
            try {
                if (request.action === 'alert_user' && !document.getElementById('phishing-alert-iframe')) {
                    showPhishingAlert(request.message);
                    sendResponse({success: true});
                } else if (request.action === 'removePopup') {
                    removePhishingAlert();
                    sendResponse({success: true});
                } else if (request.action === 'extract_features') {
                    extractFeatures();
                    sendResponse({success: true});
                }
            } catch (error) {
    ...
    function showPhishingAlert(message) {
        try {
            // Remove any existing alert
            removePhishingAlert();
            
            // Create iframe for phishing alert
            const iframe = document.createElement('iframe');
            iframe.src = chrome.runtime.getURL('popup/popup.html');
            iframe.id = 'phishing-alert-iframe';
            ...
            iframe.onload = () => {
                try {
                    iframe.contentWindow.postMessage({
                        action: 'setMessage',
                        message: message || 'This website may be a phishing attempt!'
                    }, '*');
                } catch (error) {
    ...
    function removePhishingAlert() {
        const iframe = document.getElementById('phishing-alert-iframe');
        if (iframe && iframe.parentNode) {
            iframe.parentNode.removeChild(iframe);
        }
    }
    
    // Auto-extract features when page loads
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', extractFeatures);
    } else {
        // DOM already loaded
        setTimeout(extractFeatures, 1000);
    }
